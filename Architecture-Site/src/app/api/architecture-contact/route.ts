...
const architectureContactRecipient = "kalandars2004@gmail.com";
const defaultFromEmail = "Ractysh Architecture <onboarding@resend.dev>";
...
function sourcePage(request: NextRequest, payload: ContactPayload) {
  return (
    payload.sourcePage ||
    request.headers.get("referer") ||
    request.headers.get("origin") ||
    "architecture-site/contact"
  );
}
...
function notificationRecipients() {
  return parseEmailList(
    process.env.ARCHITECTURE_CONTACT_MAIL_TO ||
      process.env.CONTACT_NOTIFICATION_EMAIL ||
      process.env.CONTACT_MAIL_TO ||
      process.env.MAIL_TO ||
      process.env.CONSULTATION_NOTIFY_TO,
    architectureContactRecipient,
  );
}

function projectMessage(payload: ContactPayload) {
  return [
...
export async function POST(request: NextRequest) {
  ...
  if (issues.length) {
    return NextResponse.json({ success: false, message: issues[0], issues }, { status: 400 });
  }

  const submittedAt = new Date().toISOString();
  const page = sourcePage(request, payload);
